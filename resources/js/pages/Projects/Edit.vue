...
<template>
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center gap-4 mb-6">
            <Link :href="`/projects/${project.id}`" class="text-gray-400 hover:text-gray-600">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            </Link>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('common.edit') }}: {{ project.name }}</h1>
        </div>
        ...
        <div class="card p-8">
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ t('projects.status') }}</label>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <Link :href="`/projects/${project.id}`" class="btn-secondary">{{ t('common.cancel') }}</Link>
